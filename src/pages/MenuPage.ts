...
export class MenuPage extends BasePage {
  ...
  private async showLeaderboard(): Promise<void> {
    ...
    const modal = new LeaderboardModal({
      ...
      onRefresh: async () => {
        const latestGroupPromise = state.player.id
          ? this.groupManager.getUserGroups(state.player.id)
          : Promise.resolve([]);
        const [latestGlobalEntries, latestTimerEntries, latestGroups] = await Promise.all([
          appwriteClient.getGlobalLeaderboard(100),
          appwriteClient.getTimerAttackLeaderboard(100),
          latestGroupPromise,
        ]);
        return {
          globalEntries: latestGlobalEntries,
          timerEntries: latestTimerEntries,
          groups: latestGroups,
        };
      },
      onOpenGroup: (group) => {
